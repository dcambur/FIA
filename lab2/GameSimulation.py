...
def group_collide(sprite_group, other_object):
    ...
    if len(remove_sprites):  # if something collided..
        sprite_group.difference_update(remove_sprites)
        an_explosion = Sprite(other_object.get_pos(), [0, 0], 0, 0,
...
def group_group_collide(rock_group, missile_group):
    ...
    for missile in list(
            missile_group):  # iterating over a copy of the missile group
...
class Ship:
    ...
    def fire(self, shoot):
        if shoot > 0:
            ...
            for i in range(
                    DIMENSIONS):  # determining position and velocity of the missile
                newpos[i] = self.pos[i] + forward_vector[i] * self.image_size[
                    i] / 2
                newvel[i] = self.vel[i] + forward_vector[i] * const_missile

            a_missile = Sprite(newpos, newvel, 0, 0, missile_image,
    ...
    def draw(self, canvas):
        if self.animated:
            new_image_center = [
                ...
                self.image_center[1]]
            canvas.draw_image(self.image, new_image_center, self.image_size,
...
def rock_spawner():
    ...
    if len(rock_group) < 12:
        ...
        if dist(newpos,
                my_ship.get_pos()) < const_rock_vicinity * my_ship.get_radius():
        ...
        a_rock = Sprite(newpos, newvel, newang, newangvel, asteroid_image,
                        asteroid_info)
...
my_ship = Ship([CANVAS_RES[0] / 2, CANVAS_RES[1] / 2], [0, 0], 0, ship_image,
               ship_info)
rock_group = set([])
missile_group = set([])
explosion_group = set([])
